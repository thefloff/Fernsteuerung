...
function fs.do_damage()
	if IsActionInRange(1) == 1 then
		PetAttack();
		local mana = UnitMana("player") / UnitManaMax("player");
		local enemyMana = UnitMana("target");
		if mana > 0.2 then
			local nrShards = fs.countItems(fs.seelensplitter);
			if nrShards > 5 and GetActionCooldown(fs.slt_seelenfeuer) == 0 then
				CastSpellByName(fs.seelenfeuer);
				return true;
			else
				CastSpellByName(fs.schattenblitz);
				return true;
			end
		elseif mana < 0.1 and enemyMana > 0.0 then
			UseAction(fs.drainMana);
			return true;
...
function fs.select_target_not_forbidden()
	fs.printDebug("Selecting next not forbidden target");
	local mark = GetRaidTargetIndex("target");
	if not fs.targetHasDebuff(fs.debuff_sheep) and
			(not mark or fs.isAllowed[mark]) then
		fs.printDebug(" -- target is good");
		return false;
	end
	return fs.selectNextTargetWithout();
end
function fs.select_target_dot_inst()
	...
	while found == 0 do
		...
		if UnitCanAttack("player", "target") and
			...
			if counter < 15 then
				if fs.targetHasDebuff(fs.dot_verderbnis) then
					fs.printDebug(" -- verderbnis present");
					if fs.targetHasDebuff(fs.dot_fluchDerPein) then
						fs.printDebug(" -- verderbnis and fluchDerPein present")
						found = 0;
					end
					if fs.targetHasDebuff(fs.debuff_fde) then
...
function fs.select_target_dot_cast()
	fs.printDebug("Selecting next target for cast dots");
	return fs.selectNextTargetWithout({fs.dot_feuerbrand})
end
function fs.select_target_damage()
...
function fs.doPrimaryOOCAction()
	...
	if fs.findItem(fs.ico_healthstone) == 0 and fs.findItem(fs.ico_seelensplitter) == 1 then
		CastSpellByName(fs.createHealthstone);
	elseif not fs.playerHasBuff(fs.buff_daemonenruestung) then
		CastSpellByName(fs.daemonenruestung);
	...
	elseif fs.countItems(fs.seelensplitter) > 20 then
		fs.dropSplitter();
	else
		FollowByName(fs.playerControlled);
	end
...
function fs.dropSplitter()
	...
	local counter = countItems(fs.ico_seelensplitter);
	...
	if counter > 5 then
		...
		do
			...
			do
				...
				do
					if GetContainerItemInfo(bag, slot) == fs.ico_seelensplitter then
